...
object ActiveJpid {
  def main(args: Array[String]) {
    val conf = new SparkConf().setAppName("ActiveJpid").setMaster("spark://bidev-cdh004:7077")
    val sc = new SparkContext(conf)
    // 注意了一定是hdfs路径。
    val lines = sc.textFile("hdfs://nameservice1/user/kaikai/mb_bootstrap_log_20170324.txt",2).cache()
